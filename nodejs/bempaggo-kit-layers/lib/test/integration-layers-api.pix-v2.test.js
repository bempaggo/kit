...
(0, vitest_1.describe)("pix", () => {
    (0, vitest_1.test)("create pix only", async () => {
        ...
        vitest_1.assert.equal(175, payment.emv.length);
    });
    ...
    (0, vitest_1.test)("create pix and get  urls of qr cod", async () => {
        ...
        const url = await setup_1.layers.getExternalQrCode(response);
        vitest_1.assert.equal(`http://localhost:5000/api/v2/charges/${response.referenceId}/qrcode`, url);
    });
    (0, vitest_1.test)("create pix and get qr cod", async () => {
        ...
        vitest_1.assert.equal(200, responseQuickResponseCode.status);
    });
});
